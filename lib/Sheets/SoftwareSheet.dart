...
class SoftwareSheet extends StatelessWidget {
  final bool isAdmin;
  const SoftwareSheet({super.key, required this.isAdmin});
...
class _SpreadsheetHomePageState extends State<SpreadsheetHomePage> {
  ...
  Future<void> _loadAllSheets() async {
    ...
    final filesJson = prefs.getString('SoftwareSheet');
  ...
  Future<void> _saveAllSheets() async {
    ...
    await prefs.setString('SoftwareSheet', encoded);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('Software Department Spreadsheets'),
...
class _ProfessionalSpreadsheetState extends State<ProfessionalSpreadsheet> {
  ...
  Future<void> _pickImage(int row, int col, ImageSource source) async {
    try {
      ...
      final fileName =
          'software_${widget.spreadsheetFile.id}_${row}_${col}_'
