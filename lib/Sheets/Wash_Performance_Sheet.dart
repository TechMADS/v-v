...
class WashSheetPage extends StatelessWidget {
  final bool isAdmin;
  const WashSheetPage({super.key, required this.isAdmin});
...
class _SpreadsheetHomePageState extends State<SpreadsheetHomePage> {
  ...
  Future<void> _loadAllSheets() async {
    ...
    final filesJson = prefs.getString('WashSheet');
  ...
  Future<void> _saveAllSheets() async {
    ...
    await prefs.setString('WashSheet', encoded);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('Wash Performance Spreadsheets'),
...
class _ProfessionalSpreadsheetState extends State<ProfessionalSpreadsheet> {
  ...
  Future<void> _pickImage(int row, int col, ImageSource source) async {
    try {
      ...
      final fileName =
          'wash_${widget.spreadsheetFile.id}_${row}_${col}_'
